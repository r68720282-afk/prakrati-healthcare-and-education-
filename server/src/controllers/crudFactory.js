export const crudFactory = (Model) => ({
  ...
  get: async (req, res) => res.json(await Model.findById(req.params.id)),
  update: async (req, res) => res.json(await Model.findByIdAndUpdate(req.params.id, req.body, { new: true })),
  remove: async (req, res) => { await Model.findByIdAndDelete(req.params.id); res.json({ message: 'Deleted' }); }
});
